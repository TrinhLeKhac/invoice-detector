...
class TestInitCapWords(unittest.TestCase):
    ...
    def test_special_characters(self):
        self.assertEqual(init_cap_words("hello-world"), "Hello-world")  # Keeps hyphenated words intact
...
class TestRemoveAccent(unittest.TestCase):
    def test_normal_vietnamese_text(self):
        ...
        self.assertEqual(remove_accent("Hà Nội là thủ đô của Việt Nam"), "Ha Noi la thu do cua Viet Nam")

    def test_vietnamese_with_uppercase(self):
        self.assertEqual(remove_accent("ĐẠI HỌC BÁCH KHOA"), "DAI HOC BACH KHOA")
        self.assertEqual(remove_accent("THÀNH PHỐ HỒ CHÍ MINH"), "THANH PHO HO CHI MINH")

    def test_vietnamese_with_mixed_case(self):
        self.assertEqual(remove_accent("Hà Nội và TP. Hồ Chí Minh"), "Ha Noi va TP. Ho Chi Minh")
...
class TestCleanDashAddress(unittest.TestCase):
    ...
    def test_standard_replacements(self):
        self.assertEqual(clean_dash_address("I live in Brvt", self.dict_norm_city_dash), "I live in Ba Ria - Vung Tau")
        self.assertEqual(clean_dash_address("Visiting Sai Gon", self.dict_norm_city_dash), "Visiting Ho Chi Minh")
        self.assertEqual(clean_dash_address("Exploring Quang Nam-Da Nang", self.dict_norm_city_dash), "Exploring Da Nang")

    def test_no_replacement_needed(self):
        self.assertEqual(clean_dash_address("I live in Hanoi", self.dict_norm_city_dash), "I live in Hanoi")

    def test_mixed_case_replacement(self):
        self.assertEqual(clean_dash_address("Enjoying hcm nightlife", self.dict_norm_city_dash), "Enjoying Ho Chi Minh nightlife")

    def test_partial_word_not_replaced(self):
        self.assertEqual(clean_dash_address("Brvton Beach is great", self.dict_norm_city_dash), "Brvton Beach is great") # pattern = r"(" + "|".join(map(re.escape, synonyms)) + r")"
...
class TestCleanAbbrevAddress(unittest.TestCase):
    ...
    def test_standard_replacement(self):
        self.assertEqual(clean_abbrev_address("Tp. Ho Chi Minh", self.dict_norm_abbrev), "Tp Ho Chi Minh")
        self.assertEqual(clean_abbrev_address("Q. 1", self.dict_norm_abbrev), "Q 1")
        self.assertEqual(clean_abbrev_address("P. Ben Nghe", self.dict_norm_abbrev), "P Ben Nghe")
    
    def test_no_change_needed(self):
        self.assertEqual(clean_abbrev_address("Ho Chi Minh City", self.dict_norm_abbrev), "Ho Chi Minh City")
    
    def test_case_insensitive_replacement(self):
        self.assertEqual(clean_abbrev_address("tp. hanoi", self.dict_norm_abbrev), "Tp hanoi")
        self.assertEqual(clean_abbrev_address("q:5", self.dict_norm_abbrev), "Q 5")
    
    def test_partial_word_not_replaced(self):
        self.assertEqual(clean_abbrev_address("Tpbank is a bank", self.dict_norm_abbrev), "Tpbank is a bank")
...
class TestCleanDigitDistrict(unittest.TestCase):
    ...
    def test_mixed_text(self):
        ...
        self.assertEqual(clean_digit_district("q 10 is a big district"), "Q10 is a big district")
...
class TestCleanDigitWard(unittest.TestCase):
    ...
    def test_no_changes_needed(self):
        self.assertEqual(clean_digit_ward("This is a normal sentence."), "This is a normal sentence.")
...
class TestRemoveSpareSpace(unittest.TestCase):
    ...
    def test_multiple_spaces(self):
        self.assertEqual(remove_spare_space("Hello    World   Again"), "Hello World Again")
    ...
    def test_mixed_spaces(self):
        self.assertEqual(remove_spare_space("  This   is   a   test  "), "This is a test")
...
class TestAddSpaceSeparator(unittest.TestCase):
    ...
    def test_mixed_cases_and_symbols(self):
        self.assertEqual(add_space_separator("hello.world,how-are_you"), "Hello World, How Are You")
        self.assertEqual(add_space_separator("HELLO_WORLD"), "Hello World")
...
class TestLastIndexOfRegex(unittest.TestCase):
    ...
    def test_special_characters(self):
        self.assertEqual(last_index_of_regex("a.b.c.d", r"\."), 5)  # Tìm vị trí của dấu '.'
        self.assertEqual(last_index_of_regex("abc$def$ghi", r"\$"), 7)  # Tìm vị trí ký tự '$'
...
class TestReplaceLastOccurrences(unittest.TestCase):
    def test_single_occurrence(self):
        self.assertEqual(replace_last_occurrences("hello world", "world", "Python"), "hello Python")
        self.assertEqual(replace_last_occurrences("test 123", "123", "456"), "test 456")

    def test_multiple_occurrences(self):
        self.assertEqual(replace_last_occurrences("abc abc abc", "abc", "xyz"), "abc abc xyz")
        self.assertEqual(replace_last_occurrences("one two three two", "two", "2"), "one two three 2")

    def test_no_occurrence(self):
        self.assertEqual(replace_last_occurrences("hello world", "Python", "Java"), "hello world")
        self.assertEqual(replace_last_occurrences("test test", "123", "456"), "test test")

    def test_replace_with_empty_string(self):
        self.assertEqual(replace_last_occurrences("abc abc abc", "abc", ""), "abc abc ")
        self.assertEqual(replace_last_occurrences("hello world world", "world", ""), "hello world ")

    def test_replace_empty_substring(self):
        self.assertEqual(replace_last_occurrences("hello world", "", "X"), "hello world")
