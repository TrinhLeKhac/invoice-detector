...
TOTAL_AMOUNT_PATTERN = rf"t[ôoòóỏọõốồổỗộơờớởỡợ]ng\s*t[iíìỉĩịîï][eéèẻẽẹêếềểễệ]n\s*h[aáàảãạăắằẳẵặâấầẩẫậ]ng{OPTIONAL_COLON}([{DIGIT_AND_MISSPELLED_CHAR}]+)"
DISCOUNT_PATTERN = rf"ch[iíìỉĩịîï][eéèẻẽẹêếềểễệ]t\s*kh[aáàảãạăắằẳẵặâấầẩẫậ][uưừứửữự]\s*h[oóòỏõọôốồổỗộơớờởỡợ][aáàảãạăắằẳẵặâấầẩẫậ]\s*[đd][oóòỏõọôốồổỗộơớờởỡợ]n{OPTIONAL_COLON}([{DIGIT_AND_MISSPELLED_CHAR}]+)"
