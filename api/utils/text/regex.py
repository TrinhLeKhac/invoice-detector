...
# COD_PATTERN1 = rf"(?:k?h?[ôoòóỏọõốồổỗộơờớởỡợ]?[mn]?[qg]?\s*th\S*[uưừứửữự]?[\.\)]*){OPTIONAL_COLON}\s*([{DIGIT_AND_MISSPELLED_CHAR}]+k?)"
COD_PATTERN1 = rf"(?:k?h?[ôoòóỏọõốồổỗộơờớởỡợ]?[mn]?[qg]?\s*th\S*[\.\)]*){OPTIONAL_COLON}\s*([{DIGIT_AND_MISSPELLED_CHAR}]+k?)"
COD_PATTERN2 = rf"([\d.,]+)\s*t[ôoòóỏọõốồổỗộơờớởỡợ]?[mn]?[qg]?\s*s[ôoòóỏọõốồổỗộơờớởỡợ]?\s*l[uưừứửữự]?[oôọóòõốồổỗộơờớởỡợ]?[mn]?[qg]?"
