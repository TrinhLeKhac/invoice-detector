...
class InvoiceTableDetector:
    def __init__(
        self, model_name="Dilipan/detr-finetuned-invoice-item-table", conf_threshold=0.5
    ):
    ...
    def detect_tables(self, image):
        ...
        for logits, boxes in zip(outputs.logits, outputs.pred_boxes):
            scores = (
                logits.softmax(-1)[..., :-1].max(-1).values
            ...
            for box, score in zip(boxes[keep], scores[keep]):
                x_min, y_min, x_max, y_max = (
                    box * torch.tensor([img_w, img_h, img_w, img_h])
                ).tolist()
                ...
                if x_max > x_min and y_max > y_min:
                    results.append(
                        {"bbox": [x_min, y_min, x_max, y_max], "score": float(score)}
                    )
